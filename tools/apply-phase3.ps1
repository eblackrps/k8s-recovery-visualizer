...
WriteUtf8NoBom ".\internal\profile\profiles.go" @(
  ...
  "}",
  "",
  "// Weights are relative multipliers applied at enrichment-time.",
  "// The keys must match CategoryScore.Name coming from recovery-scan.json categories (best effort).",
...
WriteUtf8NoBom ".\internal\enrich\enrich.go" @(
  ...
  "",
  "    // Profile selection: Options.Profile > env DR_PROFILE > standard",
  "    p := opts.Profile",
  ...
  "",
  "    // Base history comes from existing history index (overall trend)",
  ...
  "    }",
  "",
  "    // Best-effort: read recovery-scan.json categories if present",
  ...
  "    // If no weights, treat as unavailable (keeps behavior explicit)",
  "    if len(w) == 0 { return nil, nil }",
  "    // Normalize by total weighted max",
  "    totalMax := 0.0",
  "    totalGot := 0.0",
  "    for _, c := range cats {",
  "        m := c.Max",
  ...
  "    // derive posture from score only",
  ...
  "    // Load previous enrich snapshot index",
  ...
  "",
  "    // Find latest snapshot prior to current run (last entry in file)",
...
WriteUtf8NoBom ".\internal\enrich\report.go" @(
  ...
  "    // Only store category snapshots if categories exist",
  ...
  "    }{TimestampUtc: ts, SchemaVersion: en.SchemaVersion, Categories: en.Categories, Profile: en.Profile}",
  "",
